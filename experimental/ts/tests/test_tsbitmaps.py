...
from arimo.ts.tsbitmaps.tsbitmapper import TSBitMapper
from arimo.ts.tsbitmaps.bitmapviz import create_bitmap_grid


class TestBitmapAlgorithm(unittest.TestCase):

    def setUp(self):
        self.data_path = 'arimo/ts/tests/data'
    ...
    def test_anomaly_detection_ecg(self):
        ecg_norm = np.loadtxt(os.path.join(self.data_path, 'ecg_normal.txt'))
        ecg_anom = np.loadtxt(os.path.join(self.data_path, 'ecg_anom.txt'))
    ...
    def test_anomaly_detection_pattern(self):
        pattern_norm = np.loadtxt(os.path.join(self.data_path, 'pattern_normal.txt'))
        pattern_anom = pd.read_csv(os.path.join(self.data_path, 'pattern_anom.txt')).iloc[:, 0]
    ...
    def test_bitmapviz(self):
        ...
        self.assertEqual((bmp_grid > 0).sum(), sum(score >0 for score in list(ecg_bitmap.values())))
