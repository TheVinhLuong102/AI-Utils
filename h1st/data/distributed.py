...
class DistributedDataFrame(AbstractDataHandler):
    ...
    **IMPORTANT**: Any ``Spark DataFrame`` method not explicitly re-implemented in ``DistributedDataFrame``
    ...
    def select(self, *exprs, **kwargs):
        ...
        """
        if exprs:
            inheritCache = kwargs.pop('inheritCache', isinstance(exprs[0], str) and (exprs[0] == '*'))
    ...
    def distinct(self, *cols, **kwargs):
        ...
        if len(cols) > 1:
            ...
            if col in self._cache.distinct:
                ...
                if dups:
                    assert {None, - maxsize - 1}.issuperset(dups), \
                ...
                if pandas.notnull(colFallBackVal):
                    fallbackStrs = \
                        ["'{}'".format(colFallBackVal)
                         if (colType == _STR_TYPE) and isinstance(colFallBackVal, str)
    ...
    def prep(self, *cols, **kwargs):
        ...
        if vecColsToAssemble:
            if isinstance(vecColsToAssemble, str):
